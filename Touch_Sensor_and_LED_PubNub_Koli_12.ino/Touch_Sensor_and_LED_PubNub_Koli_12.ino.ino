...
const char* myID = "Koli"; // place your name here, this will be put into your "sender" value for an outgoing messsage

char publishChannel[] = "koliData"; // channel to publish YOUR data
char readChannel[] = "clintData"; // channel to read THEIR data
...
int clintPinTouched;  ///the value Koli reads from Clints board via PN
int koliPinTouched; //the value I get locally
...
if (clintPinTouched == 4) {
  for(int i=0; i<=rnd; i++) {
   pat6();}
  }

if (clintPinTouched == 6) {
  for(int i=0; i<=rnd; i++) {
   pat5();}
  }
...
void sendMessage(char channel[]) 
{
  ...
  dataToSend[JsonParamName2] = koliPinTouched; // second key value is the Pin number
    ...
    while (!getMessage.finished()) 
    {
        ...
        if (msg.length() > 0) 
        {
          ...
          deserializeJson(inMessage, msg); // parse the  JSON value received
           ...
           clintPinTouched = inMessage[JsonParamName2]; // the value of their Temperature sensor
